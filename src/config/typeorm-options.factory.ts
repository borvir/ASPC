...
export class TypeOrmOptionsFactory {
    static createTypeOrmOptions(
        ...
        const port = process.env.DATABASE_PORT
            ...
            : 5432;
        let ssl: boolean | { ca: string } = process.env.DATABASE_SSL
            ? true
            : false;
        ...
        let typeORMOptions: TypeOrmModuleOptions = {
            ...
            replication: {
                master: {
                    ...
                    port: port,
                    database: process.env.DATABASE_DB || "stockchecker",
                    username: process.env.DATABASE_USER || "stockchecker",
                    password: process.env.DATABASE_PASSWORD || "stockchecker",
                    ssl: ssl,
                },
                slaves: [
                    {
            ...
            migrationsRun:
                process.env.DATABASE_ORM_MIGRATION === "true" || false,
            logging:
